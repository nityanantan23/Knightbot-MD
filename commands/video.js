...
async function videoCommand(sock, chatId, message) {
  try {
    ...
    const apiUrl = `https://api.dreaded.site/api/ytdl/video?url=${encodeURIComponent(
      videoUrl
    )}`;
    ...
    if (!videoRes.ok) {
      await sock.sendMessage(
        chatId,
        { text: "Failed to download the video file." },
        { quoted: message }
      );
      return;
    }
    ...
    if (!buffer || buffer.length < 1024) {
      await sock.sendMessage(
        chatId,
        { text: "Downloaded file is empty or too small." },
        { quoted: message }
      );
      return;
    }
    ...
    try {
      await execPromise(
        `ffmpeg -i "${tempFile}" -c:v libx264 -c:a aac -preset fast -crf 23 -movflags +faststart "${convertedFile}"`
      );
      ...
      await sock.sendMessage(
        chatId,
        {
          ...
          mimetype: "video/mp4",
          fileName: filename,
          caption: `*${title}*\n\n> *_Downloaded by Meap Bot MD_*`,
        },
        { quoted: message }
      );
    } catch (conversionError) {
      console.log(
        "📹 Conversion failed, trying original file:",
